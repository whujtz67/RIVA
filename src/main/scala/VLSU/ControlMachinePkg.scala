...
class rowLevel(implicit p: Parameters) extends VLSUBundle {
  ...
  def init(glb_r: global): Unit = {
    this.rowBaseAddr := PriorityMux(Seq(
      glb_r.mode.Incr -> (glb_r.baseAddr + (glb_r.vstart << glb_r.wid).asUInt >> 1),
      glb_r.mode.Strd -> (glb_r.baseAddr + (glb_r.vstart *  glb_r.stride) >> 1), // This section involves adders and multipliers, TIMING crucial
    ...
    this.illuHead := PriorityMux(Seq(
      glb_r.mode.Incr -> (glb_r.vstart(0) && glb_r.EW(2)),
    ...
    this.illuTail := Mux(glb_r.EW(2), this.illuHead ^ glb_r.nrClns(0), this.illuHead)  // = this.illuHead ^ (glb_r.nrClns(0) && glb_r.EW(2))

    val offset   = this.rowBaseAddr(11, 0)
    val allBytes = (glb_r.nrClns << glb_r.wid >> 1).asUInt + (this.illuTail || this.illuHead) + offset // TODO: maybe put into the next cycle to avoid long timing path?
  ...
  def switchGrpInit(glb_nxt: global): Unit = {
    ...
    this.illuTail    := Mux(glb_nxt.EW(2), this.illuHead ^ glb_nxt.nrClns(0), this.illuHead)

    val offset   = this.rowBaseAddr(11, 0)
    val allBytes = (glb_nxt.nrClns << glb_nxt.wid >> 1).asUInt + (this.illuTail || this.illuHead) + offset
  ...
  def switchRowInit(r: rowLevel, glb_r: global): Unit = {
    this.rowBaseAddr := r.rowBaseAddr + glb_r.stride >> 1
    this.illuHead    := !r.illuHead && glb_r.stride(0)
    this.illuTail    := Mux(glb_r.EW(2), this.illuHead ^ glb_r.nrClns(0), this.illuHead)

    val offset   = this.rowBaseAddr(11, 0)
    val allBytes = (glb_r.nrClns << glb_r.wid >> 1).asUInt + (this.illuTail || this.illuHead) + offset
...
class TxnCtrlInfo(implicit p: Parameters) extends VLSUBundle {
  ...
  def init(meta_r: MetaCtrlInfo): Unit = {
    ...
    val allBytes = Mux(
      ...
      meta_r.row.ltB,       // There is only one txn in the row.
